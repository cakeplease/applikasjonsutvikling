...
open class DatabaseManager(context: Context) :
	...
	/**
	 *  Inserts the actors, the movie, director and then the connections between them.
	 */
	fun insert(actors: List<String>, movie: String, director: String) {
		writableDatabase.use { database ->
			...
			linkDirectorMovie(database, directorId, movieId)

			actors.forEach {
				val actorId = insertValueIfNotExists(database, TABLE_ACTOR, ACTOR_NAME, it)
				linkActorMovie(database, actorId, movieId)
			}
		}
	}
	 ...
	 */
	private fun linkActorMovie(database: SQLiteDatabase, actorId: Long, movieId: Long) {
		val values = ContentValues()
	...
	/**
	 *  Insert in the *TABLE_DIRECTOR_MOVIE*, essentially linking a director and a movie
	 */
	private fun linkDirectorMovie(database: SQLiteDatabase, directorId: Long, movieId: Long) {
		val values = ContentValues()
